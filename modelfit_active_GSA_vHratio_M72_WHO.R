...
C <- 0
if (C == 0) {
  home <- "/Users/lsh355020/R/M72/M72SA/"
}
if (C == 1) {
...
if (C == 0) {
  input <- "/Users/lsh355020/R/M72/M72SA/Data"
}
if (C == 1) {
...
if (C == 0) {
  outpute <- "Users/lsh355020/R/M72/M72SA/Vxoutput_M72/epi"
}
if (C == 0) {
  outputv <- "Users/lsh355020/R/M72/M72SA/Vxoutput_M72"
}
if (C == 1) {
...
setwd(home)
setwd("Data")
fitdata <- as.data.frame(drop.levels(read.csv("fitdata_sa.csv", header = TRUE, check.names = F)))
...
for (kkk in 1:rrun) {
  ...
  eee <- cbind(Xn, 0, 0, 0, 0, 0, 0, 0, 0, 0)
  colnames(eee) <- c(colnames(Xn), "type", "VE_I", "VE_D", "dur", "covH", "VEH", "covM", "mmax", "count")

  rrun_dfvx <- eee

  for (nn in 1:typen) {
    count <- 0
    coms <- matrix(0, combn, 7)
    for (vv in 1:length(effInf)) {
      for (zz in 1:length(effDis)) {
        for (xx in 1:length(durs)) {
          for (ss in 1:length(Hsafe)) {
            for (hh in 1:length(Hvax)) {
              for (ww in 1:length(coverM)) {
                for (yy in 1:length(massmax)) {
                  count <- count + 1
                  coms[count, ] <- c(effInf[vv], effDis[zz], durs[xx], Hsafe[ss], Hvax[hh], coverM[ww], massmax[yy])
                  ...
                  covM <- coverM[ww]
                  mmax <- massmax[yy]
                  print(c(nn, ticI, ticD, toc, covH, VEH, covM, mmax))

                  X <- FitGo(cntry, c(nn, cover, covM, ticI, ticD, toc, fms, vage, covH, VEH, mmax), c(p0, rmort, neta2, rmortTB, CDRscale, CDRscaleE, alpha), c(2, dt, c(0.02, 0.02, 0.8, 0.07)), c(year1, yearend), 0, C)
                  ...
                  vxtyp <- paste(vtp, "_", ticI, "_", ticD, "_", toc, "_", covH, "_", VEH, "_", covM, "_", mmax)
                  vacnames <- c(vacnames, vxtyp)

                  eee <- cbind(X, nn, ticI, ticD, toc, covH, VEH, covM, mmax, count)
                  colnames(eee) <- c(colnames(X), "type", "VE_I", "VE_D", "dur", "covH", "VEH", "covM", "mmax", "count")
  ...
  activecols <- seq(4, (((typen) * count * 3) + 1), 3)
  ...
  CAV2535 <- sum(new_active[126:136, 1]) - colSums(new_active[126:136, (activecols)])

  CAV2550 <- colSums(CAV)

  write.table(CAV2535, paste("CA_2535_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)
  write.table(CAV2550, paste("CA_2550_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)

  DAV <- matrix(0, (yearend - year1 + 1), ((typen) * count))
  DAV <- new_mort[, 1] - new_mort[, (activecols)]

  DAV2535 <- sum(new_mort[126:136, 1]) - colSums(new_mort[126:136, (activecols)])

  DAV2550 <- colSums(DAV)

  write.table(DAV2535, paste("DA_2535_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)
  write.table(DAV2550, paste("DA_2550_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)

  NVax <- matrix(0, (yearend - year1 + 1), ((typen) * count))
  NVax <- NumV[, (c(1, activecols[-(length(activecols))]))]

  NVax2550 <- colSums(NVax)

  write.table(NVax2550, paste("Nvax_2550_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)

  CAV2535all <- rbind(CAV2535all, t(as.matrix(CAV2535)))
  CAV2550all <- rbind(CAV2550all, t(as.matrix(CAV2550)))
  DAV2550all <- rbind(DAV2550all, t(as.matrix(DAV2550)))
  NVax2550all <- rbind(NVax2550all, t(as.matrix(NVax2550)))

  NNVc <- NVax2550all / CAV2550all

  write.table(NNVc, paste("NNVc_2550_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)

  NNVd <- NVax2550all / DAV2550all
  write.table(NNVd, paste("NNVd_2550_", kkk, ".csv", sep = ""), sep = ",", row.names = FALSE)
...
period_CA50_range <- matrix(0, 3, (typen * count))
period_CA35_range <- matrix(0, 3, (typen * count))
period_DA50_range <- matrix(0, 3, (typen * count))
period_NVax50_range <- matrix(0, 3, (typen * count))
period_NNVCA50_range <- matrix(0, 3, (typen * count))
period_NNVDA50_range <- matrix(0, 3, (typen * count))

for (jj in 1:(typen * count)) {
  period_CA50_range[, jj] <- c(median(CAV2550all[, jj]), min(CAV2550all[, jj]), max(CAV2550all[, jj]))
  period_CA35_range[, jj] <- c(median(CAV2535all[, jj]), min(CAV2535all[, jj]), max(CAV2535all[, jj]))
  period_DA50_range[, jj] <- c(median(DAV2550all[, jj]), min(DAV2550all[, jj]), max(DAV2550all[, jj]))
  period_NVax50_range[, jj] <- c(median(NVax2550all[, jj]), min(NVax2550all[, jj]), max(NVax2550all[, jj]))
  period_NNVCA50_range[, jj] <- c(median(NNVc[, jj]), min(NNVc[, jj]), max(NNVc[, jj]))
  period_NNVDA50_range[, jj] <- c(median(NNVd[, jj]), min(NNVd[, jj]), max(NNVd[, jj]))
}

period_CA50_range <- t(round(period_CA50_range))
period_CA35_range <- t(round(period_CA35_range))
period_DA50_range <- t(round(period_DA50_range))
period_NVax50_range <- t(round(period_NVax50_range))
period_NNVCA50_range <- t(round(period_NNVCA50_range))
period_NNVDA50_range <- t(round(period_NNVDA50_range))

period_CA50_range <- as.data.frame(cbind(period_CA50_range, vacnames))
period_CA35_range <- as.data.frame(cbind(period_CA35_range, vacnames))
period_DA50_range <- as.data.frame(cbind(period_DA50_range, vacnames))
period_NVax50_range <- as.data.frame(cbind(period_NVax50_range, vacnames))
period_NNVCA50_range <- as.data.frame(cbind(period_NNVCA50_range, vacnames))
period_NNVDA50_range <- as.data.frame(cbind(period_NNVDA50_range, vacnames))

write.table(period_CA50_range, "period_CA_range_2050.csv", sep = ",", row.names = FALSE)
write.table(period_CA35_range, "period_CA_range_2035.csv", sep = ",", row.names = FALSE)
write.table(period_DA50_range, "period_DA_range_2050.csv", sep = ",", row.names = FALSE)
write.table(period_NVax50_range, "period_NVax50_range.csv", sep = ",", row.names = FALSE)
write.table(period_NNVCA50_range, "period_NNVCA_range_2050.csv", sep = ",", row.names = FALSE)
write.table(period_NNVDA50_range, "period_NNVDA_range_2050.csv", sep = ",", row.names = FALSE)

setwd(home)
source("#WHOplots_SA.R")

setwd(home)
setwd(home)
setwd("Vxoutput_M72/epi")
setwd("1.epi")
...
fitcheck <- rbind(yrTBItot[c(1, 17), ], yrTBI_014[17, ], yrTBI_15[17, ], yrTBIHtot[c(1, 17), ], yrTBMtot[c(1, 16), ], yrTBMHtot[c(1, 16), ], yrTBNtot[c(1, 16), ], yrTBN_014[16, ], yrTBN_15[16, ], yrTBNHtot[16, ], yrTBPtot[16, ])

fitkeep <- c()

for (i in 1:ncol(fitcheck)) {
  if (((fitcheck[1, i] > fitdata[1, 4]) & (fitcheck[2, i] > fitdata[2, 4]) & (fitcheck[3, i] > fitdata[3, 4]) &
    (fitcheck[4, i] > fitdata[4, 4]) & (fitcheck[5, i] > fitdata[5, 4]) & (fitcheck[6, i] > fitdata[6, 4]) &
    (fitcheck[7, i] > fitdata[7, 4]) & (fitcheck[8, i] > fitdata[8, 4]) & (fitcheck[9, i] > fitdata[9, 4]) &
    (fitcheck[10, i] > fitdata[10, 4]) & (fitcheck[11, i] > fitdata[11, 4]) & (fitcheck[12, i] > fitdata[12, 4]) &
    (fitcheck[13, i] > fitdata[13, 4]) & (fitcheck[14, i] > fitdata[14, 4]) & (fitcheck[15, i] > fitdata[15, 4]) & (fitcheck[16, i] > fitdata[16, 4])) == TRUE) {
    fitkeep <- rbind(fitkeep, para[i, ])
  }
}

write.table(fitcheck, "fitcheck_first1000.csv", sep = ",", row.names = F)
write.table(fitkeep, "fitkeep_fromfirst1000.csv", sep = ",", row.names = F)
write.table(fitkeep, "fitkeep_fromsecond250.csv", sep = ",", row.names = F)

fitkeep2 <- fitkeep
fitkeep3 <- rbind(fitkeep2, fitkeep[1:122, ])

write.table(fitkeep3, "fit1000SA_HARTdeathfix.csv", sep = ",", row.names = F)
...
prevLTBI <- as.numeric(as.data.table(prevLTBI))
med_TBPIage <- matrix(0, 3, 101)
